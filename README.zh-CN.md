<div align="center">
  <img src="./assets/branding/noritrans.svg" width="112" alt="NoriTrans 标志" />
  <h1>NoriTrans</h1>
...
> [!IMPORTANT]
> NoriTrans 是仍在积极开发的 `0.x` 项目。目前正式面向桌面 Chrome 138+ 和 Manifest V3。流媒体网站修改播放器或私有接口后，站点专用字幕适配可能需要同步维护。

## 为什么做 NoriTrans？

NoriTrans 把网页翻译、视频字幕翻译和可选的本地 OCR 放在一个扩展中，但不会把三种问题混成同一条不透明流程。
...
NoriTrans 会优先使用完整来源，再逐级回退到当前字幕采集：
...
1. 从 [GitHub Releases](https://github.com/Norixor/NoriTrans/releases) 下载最新版本的 ZIP 安装包；
...
NoriTrans 基于 [WXT](https://github.com/wxt-dev/wxt)、`idb`、ONNX Runtime Web、Bergamot 和兼容 PaddleOCR 的本地运行时构建。OCR 与本地翻译运行时的第三方说明随对应打包资源保存。
