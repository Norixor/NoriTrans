declare global {
  interface ImportMetaEnv {
    readonly WXT_NORITRANS_SITE_DIAGNOSTICS?: string;
  }
}

export function siteDiagnosticsEnabled(): boolean {
  return import.meta.env.WXT_NORITRANS_SITE_DIAGNOSTICS === "1";
}
...
export function siteDiagnostic(
  ...
  console.info(`[NoriTrans][${site}] ${event}`, detail);
}
...
export function translationDiagnostic(
  ...
  console[level](`[NoriTrans][${scope}] ${event} ${JSON.stringify(detail)}`);
}
